...
int MedianFilter::mode(byte size) // return the value of the average of samples near the median
{
        long sum = 0;
        byte count;
	int ix;

	if (size > medFilterWin)
	  return  sortedData[medDataPointer];     

	for(count = 0, ix = medDataPointer - (size/2); count < size; ++count, ++ix) {
	  sum += sortedData[ix];
	}
	return sum/size;
}
