...
uint16_t Adafruit_ADS1015::readADC_SingleEnded(uint8_t channel) {
  ...
  // Select Conversion Complete function for ALERT
  writeRegister(m_i2cAddress, ADS1015_REG_POINTER_LOWTHRESH, 0);
  writeRegister(m_i2cAddress, ADS1015_REG_POINTER_HITHRESH, 0x8000);  
  config &= ~(ADS1015_REG_CONFIG_CQUE_MASK | ADS1015_REG_CONFIG_CPOL_MASK);
  config |=  (ADS1015_REG_CONFIG_CQUE_1CONV | ADS1015_REG_CONFIG_CPOL_ACTVLOW);
...
int16_t Adafruit_ADS1015::readADC_Differential_0_1() {
  ...
  uint16_t config = ADS1015_REG_CONFIG_CQUE_NONE    | // Disable the comparator (default val)
                    ...
                    ADS1015_REG_CONFIG_CMODE_TRAD   | // Traditional comparator (default val)
  //                ADS1015_REG_CONFIG_DR_1600SPS   | // 1600 samples per second (default)
                    ADS1015_REG_CONFIG_DR_3300SPS   | // 250 samples per second 
                    ADS1015_REG_CONFIG_MODE_SINGLE;   // Single-shot mode (default)
  ...
  //delay(m_conversionDelay);
  for (;;) {
    if (readRegister(m_i2cAddress, ADS1015_REG_POINTER_CONFIG) & ADS1015_REG_CONFIG_OS_SINGLE)
      break;
  }
